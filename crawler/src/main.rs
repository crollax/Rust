...
pub fn get_links(url: &Url, page: String) -> Vec<Url> {
  ...
  // Identify tokens in HTML document
  let mut tokenizer = Tokenizer::new(&mut queue, TokenizerOpts::default());

  // Reads through chars in HTML page
  let mut buffer = BufferQueue::new();
  buffer.push_back(page.into()); // push_back adds to front of buffer queue
  let _ = tokenizer.feed(&mut buffer);

  // Iterate through links and map-match valid urls
  queue
    .links
    .iter()
...
// Makeing crawl call efficient
  // Put into Box pointer
